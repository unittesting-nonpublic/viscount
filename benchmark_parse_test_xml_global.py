...
log_paths = []
# Create 1 large log to demonstrate quadratic slowdown from global concatenation across test files if not optimized further
# The previous benchmark reset parse_test_xml.df for the suite, but if there were 10 logs with 10k testcases total it tests it well.
# Let's see if 10000 testcases in one log makes a bigger difference.
for i in range(1):
    testcases = "".join([testcase_template.format(i, j, j, j) for j in range(10000)])
